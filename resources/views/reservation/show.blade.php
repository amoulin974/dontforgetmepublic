        ...
        <div class="entreprise">
            <div class="res-info" style="text-align: center">
                <p>{{__('Booked at ')}}<strong>{{ $reservation->effectuer_activites()->wherePivot('idUser',Auth::user()->id)->first()->entreprise->libelle }}</strong>
                   ...
                   {{__('From')}} <strong>{{ explode(':',$reservation->heureDeb)[0] }}h{{ explode(':',$reservation->heureDeb)[1] }}</strong> {{__('to')}} <strong>{{ explode(':',$reservation->heureFin)[0] }}h{{ explode(':',$reservation->heureFin)[1] }}</strong>.</p>
